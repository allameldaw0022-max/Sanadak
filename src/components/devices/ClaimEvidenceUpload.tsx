...
import { submitClaimEvidenceAction } from "@/app/devices/claims/actions";

const IMAGE_TYPES = ["image/png", "image/jpeg", "image/webp"];
const MAX_SIZE = 5 * 1024 * 1024;

// Fast, non-authoritative pre-check for instant feedback -- the server
// always re-validates by real bytes (see image-validation.ts) regardless.
...
export function ClaimEvidenceUpload({ claimId }: { claimId: string }) {
  ...
  function handleFile(file: File) {
    ...
    startTransition(async () => {
      const result = await submitClaimEvidenceAction(claimId, file);
      if (!result.ok) {
    ...
    <div>
      ...
      >
        ...
        <p className="text-xs font-semibold text-slate-500">
          {pending ? "جارٍ الرفع..." : "اضغط أو اسحب صورة الدليل هنا (إيصال شراء، صورة تُظهر IMEI...)"}
        </p>
      </div>
